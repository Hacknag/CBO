...
def insertar_reabastecimiento(id_reabastecimiento, cantidad, fecha_llegada, id_estado):
    conexion = get_connection()
    try:
        cursor = conexion.cursor()
        cursor.callproc("FIDE_PROYECTO_FINAL_PKG.FIDE_REABASTECIMIENTO_INVENTARIO_INSERT_SP", [
            id_reabastecimiento, cantidad, fecha_llegada, id_estado
        ])
        conexion.commit()
...
def actualizar_reabastecimiento(id_reabastecimiento, cantidad, fecha_llegada, id_estado):
    conexion = get_connection()
    try:
        cursor = conexion.cursor()
